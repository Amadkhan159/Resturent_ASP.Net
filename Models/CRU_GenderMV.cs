...
namespace Resturent.Models
{
    public class CRU_GenderMV
    {
        ...
        {
            GetAllGenders();
            var editgender = new ResturentDbEntities().GenderTables.Where(g => g.GenderID == id).FirstOrDefault();
            if (editgender != null)
            {
        ...
        private void GetAllGenders()
        {
            ...
            foreach (var gender in new ResturentDbEntities().GenderTables.ToList())
            {
